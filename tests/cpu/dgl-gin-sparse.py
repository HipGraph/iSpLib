# python dgl-gin-sparse.py reddit isplib
# python dgl-gin-sparse.py reddit pt2 nopad
...
print(f'Running GINConv, Epoch: {EPOCH_COUNT}, Embedding: {EMBEDDING_SIZE}, Dataset: {sys.argv[1]}, Mode: DGL, Padding: {padding}')
...
class GINNet(torch.nn.Module):
    ...
    def forward(self, g, features):
        x = self.bn1(self.conv1(g, features))
        x = self.bn2(self.conv2(g, x))
        x = self.fc1(x).relu()
        return self.fc2(x)
