...
import pytest
import torch
from the_sparse_package.storage import SparseStorage
...
def test_ind2ptr(device):
    ...
    rowptr = torch.ops.the_sparse_package.ind2ptr(row, 8)
    assert rowptr.tolist() == [0, 0, 0, 2, 2, 3, 5, 6, 6]

    row = torch.ops.the_sparse_package.ptr2ind(rowptr, 6)
    ...
    row = torch.ops.the_sparse_package.ptr2ind(rowptr, 0)
    assert row.tolist() == []
