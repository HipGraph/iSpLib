...
from the_sparse_package.utils import Final
...
class SparseStorage(object):
    ...
    def row(self):
        ...
        rowptr = self._rowptr
        if rowptr is not None:
            row = torch.ops.the_sparse_package.ptr2ind(rowptr, self._col.numel())
            self._row = row
            return row
    ...
    def rowptr(self) -> torch.Tensor:
        ...
        row = self._row
        if row is not None:
            rowptr = torch.ops.the_sparse_package.ind2ptr(row, self._sparse_sizes[0])
            self._rowptr = rowptr
            return rowptr
    ...
    def colptr(self) -> torch.Tensor:
        ...
        csr2csc = self._csr2csc
        if csr2csc is not None:
            colptr = torch.ops.the_sparse_package.ind2ptr(self._col[csr2csc],
