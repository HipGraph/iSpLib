...
public:
  static variable_list forward(AutogradContext *ctx,
    ...
    ctx->saved_data["has_value"] = has_value;
    ctx->save_for_backward({row, rowptr, col, value, rowcount, colptr, csr2csc, mat});
    return {out};
  }
...
//   return fusedmm_spmm_fw(rowptr, col, value, mat);
  return FusedMM_SPMMMean::apply(opt_row, rowptr, col, value, opt_rowcount, opt_colptr, opt_csr2csc, mat, opt_value.has_value())[0];
}
